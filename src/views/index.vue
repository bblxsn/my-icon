      ...
      <el-space wrap>
        ...
        >
          <el-image
              ...
              @click="handleClick(item.path ,item.name + '.' +item.suffix)"

          />
          <div class="card_txt" @click="handleGiteeClick(item.path ,item.name + '.' +item.suffix)">
            {{ item.name }}
          </div>
        </div>
      </el-space>
  ...
  setup() {
    ...
    function readLocalFile() {
      return new Promise((resolve, reject) => {
        fetch(data.publicPath + "db3.json?" + new Date().getTime())
    ...
    async function handleSelect(value) {
      ...
      if (value == 1) {
        data.selectlabel = "容器";
        filteredData = tempdata.filter((item) => item.type == "docker");
      } else if (value == 2) {
        data.selectlabel = "虚拟机";
        filteredData = tempdata.filter((item) => item.type == "vms");
      } else if (value == 3) {
        data.selectlabel = "其他";
        filteredData = tempdata.filter((item) => item.type == "other");
      } else if (value == 4) {
        data.selectlabel = "圆角矩形图标";
        filteredData = tempdata.filter((item) => item.icon_type == "border-radius");
      } else if (value == 5) {
        data.selectlabel = "圆形图标";
        filteredData = tempdata.filter((item) => item.icon_type == "circle");
      } else if (value == 6) {
        data.selectlabel = "SVG 图标";
        filteredData = tempdata.filter((item) => item.suffix == "svg");
      } else {
        data.selectlabel = "全部";
    ...
    //按钮
    async function handleClick(path, url) {
      //获取图片url
      let currenturl = window.location.href;
      let iconurl = data.publicPath + path +"/" + url;
    ...
    //按钮
    async function handleGiteeClick(path, url) {
      //获取图片url
